...
Three details that decide whether this works (Ch 20.5):
...
def dpo_loss(policy_chosen_logps: torch.Tensor,
    ...
    The "rewards" are the implicit reward beta*log(pi/pi_ref) from Ch 20.3 —
...
def simpo_loss(policy_chosen_logps, policy_rejected_logps,
    ...
    """
    SimPO — Ch 21.1. Length-normalised, reference-free, with an explicit margin.
...
def build_dpo_batch(tokenizer, prompt: str, chosen: str, rejected: str, device=None):
    ...
    def pack(response: str):
        ...
        labels = ids.clone()
        labels[: len(p)] = -100                   # Ch 19.2 — mask the prompt
        return ids, labels
...
def precompute_reference_logps(ref_model, batches):
    """
    Ch 20.5: the reference never changes, so compute its log-probs ONCE for the
